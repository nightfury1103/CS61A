...
class VendingMachine:
    ...
    def restock(self, amount):
        self.stock += amount
        print("'Current {:s} stock: {:d}'".format(self.product, self.stock))

    def deposit(self, amount):
        if self.stock == 0:
            print("'Machine is out of stock. Here is your $%d.'" % amount)
        else:
            self.balance += amount
    ...
    def vend(self):
        if self.stock == 0:
            ...
            print("'You must deposit $%d more.'" % (self.price - self.balance))
        elif self.balance == self.price:
            self.balance = 0
            self.stock -= 1
            print("'Here is your %s.'" % self.product)
        else:
            print("'Here is your {:s} and ${:d} change.'".format(self.product, self.balance - self.price))
            self.balance = 0
            self.stock -= 1
...
class MissManners:
    ...
    def ask(self, message, *args):
        ...
        if not message.startswith(magic_word):
        ...
        elif type(self.obj) == MissManners:
            ...
            return 'Thanks for asking, but I know not how to {0}.'.format(message.strip('please '))
